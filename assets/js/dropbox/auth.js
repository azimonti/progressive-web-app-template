...
import { CLIENT_ID, REDIRECT_URI, ACCESS_TOKEN_KEY, REFRESH_TOKEN_KEY } from './config.js?id=fc364d';
import { logVerbose } from '../logging.js?id=fc364d';
import { updateAuthButton, updateSyncIndicator, SyncStatus } from './ui.js?id=fc364d';
import { getKnownFiles, addKnownFile } from '../storage/storage.js?id=fc364d';
import { initializeDropboxApi, getDbxInstance } from './api.js?id=fc364d';
import { updateFileSelectionUI } from '../storage/files.js?id=fc364d';

const CODE_VERIFIER_KEY = 'dropboxCodeVerifier'; // Key for sessionStorage

let dbxAuth = null;
let accessToken = localStorage.getItem(ACCESS_TOKEN_KEY);
let refreshToken = localStorage.getItem(REFRESH_TOKEN_KEY); // Load refresh token
...
export async function initializeAuthentication() {
  ...
  if (accessToken && !redirected) {
    ...
    updateFileSelectionUI(); // Explicitly update UI after discovery
  }
...
async function discoverDropboxFiles() {
  ...
  try {
    ...
    // Removed UI update call from here - it's now called after discoverDropboxFiles completes
    if (filesAdded) {
      logVerbose('New files added from Dropbox discovery.');
    } else {
